...
_BASE_SCRIPT: list[tuple[str, str | None]] = [
    ...
    (
        "assistant",
        "Existing tests don't cover the boundary case. I'll write "
        "a fix and add a targeted test.",
    ),
...
def generate_long_conversation(num_turns: int = 50) -> list[ConversationTurn]:
    ...
    for i in range(num_turns):
        if i in _ANCHOR_DECISIONS:
            ...
            turns.append(
                ConversationTurn(turn_number=i, role="user", content=user_msg)
            )
            turns.append(
                ConversationTurn(
                    turn_number=i, role="assistant", content=assistant_msg
                )
            )
            ...
            if role == "tool":
                ...
                turns.append(
                    ConversationTurn(turn_number=i, role=role, content=text)
                )
        else:
