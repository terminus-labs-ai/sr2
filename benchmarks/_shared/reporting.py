...
def print_token_table(turns: list[TurnMetrics]) -> None:
    ...
    print(f"  (Early turns: managed is larger because it includes retrieved memories)")
    print()
...
def print_budget_compliance(
    ...
    print(f"  Naive exceeded budget:    {len(naive_over):>3}/{len(turns)} turns  ({naive_pct:.0f}%)")
    print(f"  Managed exceeded budget:  {len(managed_over):>3}/{len(turns)} turns  ({managed_pct:.0f}%)")

    if naive_over:
        worst_naive = max(t.naive_tokens for t in turns)
        print(f"  Naive peak:               {worst_naive:>12,} tokens  ({worst_naive / token_budget:.1f}x budget)")
    ...
    print(f"  Managed peak:             {worst_managed:>12,} tokens  ({utilization:.0f}% of budget)")

    if not managed_over:
        print(f"  -> Managed context never exceeded the token budget")
    print()
...
def print_compile_overhead(turns: list[TurnMetrics]) -> None:
    ...
    print(f"  -> Pipeline adds negligible latency to each turn")
    print()
...
def print_headline_metrics(
    ...
    if token_budget:
        ...
        print(f"  * Budget compliance: naive {len(turns) - naive_over}/{len(turns)}, "
    ...
    if turns:
        ...
        print(f"  * {avg_us:.0f}us avg compile overhead ({avg_us / 500_000 * 100:.3f}% of a 500ms LLM call)")
