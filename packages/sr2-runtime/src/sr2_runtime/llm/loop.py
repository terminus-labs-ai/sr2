...
class LLMLoop:
    ...
    async def run(
        ...
        for i in range(self._max_iter):
            ...
            if not llm_response.has_tool_calls:
                if llm_response.raw_tool_call_text:
                    ...
                    logger.warning(
                        "Model emitted hallucinated tool call, retrying without tools (attempt %d/3)",
                        hallucination_retries,
                    )
        ...
        if llm_response and (
            llm_response.raw_tool_call_text
            or (llm_response.has_tool_calls and not llm_response.content)
        ):
            logger.warning("Reached max iterations with pending tool call, forcing final synthesis")
            try:
                llm_response = await self._llm.complete(
                    messages=messages,
                    tools=None,
