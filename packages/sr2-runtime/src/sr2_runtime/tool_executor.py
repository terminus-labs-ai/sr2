...
class RecallMemoryTool:
    ...
    def tool_definition(self) -> dict:
        ...
        if self._key_schema:
            prefixes = ", ".join(
                f"'{(e['prefix'] if isinstance(e, dict) else e.prefix).rstrip('.')}'"
                for e in self._key_schema
            )
        ...
        return {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": ("What you're looking for — a natural language description"),
                    },
                    "key_prefix": {
                        "type": "string",
                        "description": (
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if key_prefix and not query:
            ...
            results = [
                MemorySearchResult(memory=mem, relevance_score=1.0, match_type="key_match")
            ...
            if key_prefix:
                results = [r for r in results if r.memory.key.startswith(key_prefix)][:top_k]
