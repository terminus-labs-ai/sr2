...
from sr2_runtime.mcp import MCPManager, MCPToolHandler
...
class TestMCPManager:
    ...
    async def test_discover_all_discovers_and_registers_tools(self):
        ...
        with (
            patch("sr2_runtime.mcp.client._MCP_AVAILABLE", True),
            patch("sr2_runtime.mcp.client.StdioServerParameters", create=True),
            patch("sr2_runtime.mcp.client.stdio_client", create=True, return_value=mock_cm),
            patch("sr2_runtime.mcp.client.ClientSession", create=True, return_value=mock_session),
        ):
    ...
    async def test_make_transport_stdio(self):
        ...
        with (
            patch("sr2_runtime.mcp.client.StdioServerParameters", create=True) as mock_params,
            patch("sr2_runtime.mcp.client.stdio_client", create=True) as mock_stdio,
        ):
    ...
    async def test_make_transport_http(self):
        ...
        with patch("sr2_runtime.mcp.client.streamablehttp_client", create=True) as mock_http:
            MCPManager._make_transport(config)
